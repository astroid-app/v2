...
@api.get("/", description="Does nothing. Just sitting there and waiting for requests. (Redirects on 'GET' to the "
                          "documentation.)")
def root():
    return fastapi.responses.RedirectResponse(status_code=301, url="https://astroid.deutscher775.de/docs")

@api.get("/invite/{platform}", description="Get the invite link for the astroid bot.")
def invite(platform: str):
    if platform == "discord":
        ...
        if token is not None:
            if token == Bot.config.MASTER_TOKEN or token == data_token:
                if f"{endpoint}.json" in os.listdir(f"{pathlib.Path(__file__).parent.resolve()}/endpoints"):
                    ...
                    if sender:
                        if sender == "discord" or sender == "guilded" or sender == "revolt":
                        ...
                        if "," in allowed_ids:
                            for val in allowed_ids.split(","):
                                if val in json_file["config"]["allowed-ids"]:
                                    ...
                                    try:
                                        json_file["config"]["allowed-ids"].append(int(val))
                                    except ValueError:
                            ...
                            if allowed_ids in json_file["config"]["allowed-ids"]:
                                ...
                                try:
                                    json_file["config"]["allowed-ids"][index] = int(allowed_ids)
                                except ValueError:
                                    json_file["config"]["allowed-ids"][index] = allowed_ids
                            else:
                                try:
                                    json_file["config"]["allowed-ids"].append(int(allowed_ids))
                                except ValueError:
                    ...
                    if message_attachments:
                        if "," in message_attachments:
                            ...
                            for attachment in json_file["meta"]["message"]["attachments"] and sender == "guilded":
                                json_file["meta"]["message"]["content"] = attachment.replace(attachment, "")

                    if message_embed:
                        if not "http" in message_content and not "https" in message_content:
                    ...
                    if updated_json["config"]["self-user"] is False:
                        ...
                        if sender == "discord":
                                ...
                                for channel in updated_json["config"]["channels"]["discord"]:
                                    if str(channel) == updated_json["meta"]["sender-channel"]:
                                        ...
                                        if message_content or message_embed:
                                            if message_embed:
                                                gc_embed = updated_json["meta"]["message"]["embed"]
                                                embed = guilded.Embed(title=gc_embed["title"], description=gc_embed["title"])
                                                if len(gc_embed["fields"]) != 0:
                                                    for field in gc_embed["fields"]:
                                                        embed.add_field(name=field["name"], value=field["value"], inline=field["inline"])
                                                if gc_embed["image"]:
                                                    embed.set_image(url=gc_embed["image"])
                                                if gc_embed["thumbnail"]:
                                                    embed.set_thumbnail(url=gc_embed["thumbnail"])
                                                if gc_embed["footer"]:
                                ...
                                for channel in updated_json["config"]["channels"]["guilded"]:
                                    if str(channel) == updated_json["meta"]["sender-channel"]:
                                        ...
                                        if message_content or message_embed:
                                            if message_embed:
                                                gc_embed = updated_json["meta"]["message"]["embed"]
                                                embed = nextcord.Embed(title=gc_embed["title"], description=gc_embed["title"])
                                                if len(gc_embed["fields"]) != 0:
                                                    for field in gc_embed["fields"]:
                                                        embed.add_field(name=field["name"], value=field["value"], inline=field["inline"])
                                                if gc_embed["image"]:
                                                    print(gc_embed["image"].replace("!PARAM", "&"))
                                                    embed.set_image(url=gc_embed["image"].replace("!PARAM", "&").split("?")[0].replace("?", ""))
                                                if gc_embed["thumbnail"]:
                                                    print(gc_embed["thumbnail"].replace("!PARAM", "&"))
                                                    embed.set_thumbnail(url=gc_embed["thumbnail"].replace("!PARAM", "&"))
                                                if gc_embed["footer"]:
                                                    embed.set_footer(text=gc_embed["footer"])
                            ...
                            if not updated_json["config"]["channels"]["discord"]:
                                updated_json["meta"]["read"]["discord"] = True
                            elif str(updated_json["meta"]["sender-channel"]) in updated_json["config"]["channels"][
                                "revolt"]:
