...
client = commands.CommandsClient(prefix="gc!")


def get_endpoint(server: voltage.Server):
    print(server.id)
    try:
...
async def on_ready():
    ...
    await client.set_status(text="gc!help | Watching your server")
    while True:
        for server in os.listdir(f"{pathlib.Path(__file__).parent.resolve()}/revolt_servers"):
            try:
                    ...
                    if message_attachments:
                        if not content is None:
                            await channel.send(content=content,
                                               masquerade=voltage.MessageMasquerade(name=author_name,
                        ...
                        await channel.send(content=content,
                                           masquerade=voltage.MessageMasquerade(name=author_name, avatar=author_avatar,
                                                                                colour=color))
                        post = requests.post(
                            f"https://guildcord-api.tk/read/{endpoint}?token={config.MASTER_TOKEN}&read_revolt=true")
                        post.close()
                ...
                print("Invalid json")
                pass
            except voltage.errors.HTTPError:
                traceback.print_exc()
...
async def on_message(message: voltage.Message):
    if not message.author.bot:
        if not message.content.lower().startswith(f"{client.prefix}help"):
            await client.handle_commands(message)
        if message.content.lower() == f"{client.prefix}help":
            embed = voltage.SendableEmbed(title="Guildcord", description="API Docs: https://guildcord-api.tk/docs\n\n"
                ...
                for word in blacklist:
                    if word is not None:
                        if word.lower() in message.content.lower():
                            embed = voltage.SendableEmbed(title=f"{message.author.name} - Flagged",
                                                          description=message.content)
                            channel_request = requests.get(
                                f"https://guildcord-api.tk/{endpoint}?token={config.MASTER_TOKEN}")
                            channel_json = channel_request.json()["config"]["logs"]["revolt"]
                            channel = client.get_channel(channel_json)
...
async def register(ctx: commands.CommandContext, endpoint: int = None):
    if endpoint is None:
    ...
    try:
        ...
        await ctx.reply(f"Enpoint exists already: https://guildcord-api.tk/{endpoint}")
        return
    # webhook = await message.channel.create_webhook(name="Guildcord")
    requests.post(
        f"https://guildcord-api.tk/update/{endpoint}?channel_revolt={ctx.message.channel.id}&"
        f"&token={config.MASTER_TOKEN}")
    await ctx.reply(f"Updated enpoint: https://guildcord-api.tk/{endpoint}")


@client.command(name="allow")
async def allow(ctx: commands.CommandContext, allowid: str = None):
    if allowid is None:
...
@client.command(name="set-logs")
async def set_logs(ctx: commands.CommandContext, channel: str = None):
    if channel is None:
    ...
    if endpoint:
        requests.post(
            f"https://guildcord-api.tk/update/{endpoint}?"
...
@client.command(name="add-bridge")
async def set_logs(ctx: commands.CommandContext):
    endpoint = get_endpoint(ctx.server)
    if endpoint:
        requests.post(
            f"https://guildcord-api.tk/update/{endpoint}?channel_revolt={ctx.channel.id}&token={config.MASTER_TOKEN}")
        await ctx.reply(f"Updated enpoint: https://guildcord-api.tk/{endpoint}")
    else:
        await ctx.reply(":x: - Your endpoint could not be found.")


client.run(token=config.REVOLT_TOKEN)
